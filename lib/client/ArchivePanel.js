...
export function ArchiveSection(props) {
    ...
    return (_jsxs("div", { className: C.archive, children: [_jsx("div", { className: C.archiveHeader, children: _jsx("div", { className: C.archiveDesc, children: t("archive.description") }) }), _jsxs("div", { className: C.archiveToolbar, children: [_jsxs("button", { type: "button", className: C.archiveSelectAll, onClick: toggleSelectAllOnPage, disabled: pageRows.length === 0, children: [_jsx("input", { type: "checkbox", className: C.archiveCheckbox, checked: allOnPageSelected, readOnly: true }), t("archive.select_all")] }), _jsxs("button", { type: "button", className: `${C.archiveDeleteBtn} ${C.archiveDeleteBtnDanger}`, onClick: handleDeleteClick, disabled: selected.size === 0 || deleting, children: [t("archive.delete_selected"), " (", selected.size, ")"] }), totalPages > 1 && (_jsxs("div", { className: C.archivePager, children: [_jsx("button", { type: "button", className: C.archivePageBtn, onClick: () => setPage((p) => Math.max(0, p - 1)), disabled: currentPage === 0, children: "\u2039" }), _jsxs("span", { className: C.archivePageInfo, children: [currentPage + 1, " / ", totalPages] }), _jsx("button", { type: "button", className: C.archivePageBtn, onClick: () => setPage((p) => Math.min(totalPages - 1, p + 1)), disabled: currentPage >= totalPages - 1, children: "\u203A" })] }))] }), message !== null && _jsx("div", { className: C.archiveMessage, children: message }), allRows.length === 0 ? (_jsx("div", { className: C.archiveMessage, children: t("archive.empty") })) : (_jsx("div", { className: C.archiveGroups, children: grouped.map(([wsLabel, rows]) => {
                ...
                }) })), _jsx(Modal, { open: showConfirm, onClose: () => {
                    if (!deleting)
                        setShowConfirm(false);
                }, title: t("archive.confirm_title"), closeLabel: t("archive.close"), description: t("archive.confirm"), className: C.archiveDeleteDialog, footer: _jsxs(_Fragment, { children: [_jsx(Button, { variant: "outline", autoFocus: true, disabled: deleting, onClick: () => setShowConfirm(false), children: t("archive.cancel") }), _jsx(Button, { variant: "outline", className: C.archiveDeleteConfirm, disabled: deleting, onClick: () => void confirmDelete(), children: t("archive.confirm_delete") })] }) })] }));
}
