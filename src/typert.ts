		...
		services: [
			{
				...
				types: [
					{ name: "SessionId", declaration: "export type SessionId = string;" },
					{
						name: "PinRecord",
						declaration:
							"export interface PinRecord { id: string; pinnedAt: number; }",
					},
					{
						name: "ToggleResult",
						declaration:
							"export interface ToggleResult { id: string; pinned: boolean; }",
					},
					{
						name: "DeleteResult",
						declaration:
							"export interface DeleteResult { deleted: string[]; errors: { id: string; error: string }[]; }",
					},
				],
			},
		],
